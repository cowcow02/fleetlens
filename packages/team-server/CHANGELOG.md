...
### Changed
- **Demo insights data is fully generic.** The sample report behind `?mock=1` now uses fictional placeholder projects, tickets, and member names throughout.
...
### Fixed
- **Docker image build (retry).** Stage Playwright packages by filesystem walk (`find …/playwright-core/package.json`) instead of `require.resolve`, which kept failing under pnpm isolation in CI. Ships as **0.15.6**.
...
### Fixed
- Pairing panel no longer suggests `brew install fleetlens` — the CLI ships via npm only.
