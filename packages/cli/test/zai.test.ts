 ...
 * real Z.ai API and WITHOUT relying on any real credential. CI has no
 * ZAI_API_KEY, so every test injects a fake key via env and mocks fetch.
 */
...
describe("fetchZaiUsage", () => {
  it("throws no_key when no credential is available", async () => {
    // CI may have no key, but a dev shell might — clear it so the test is
    // deterministic regardless of the environment.
    delete process.env.ZAI_API_KEY;
    delete process.env.GLM_API_KEY;
    // Point HOME at a temp dir so the ~/.config key-file candidates don't
    // resolve to a real key on this machine and mask the no_key path.
    const prevHome = process.env.HOME;
    process.env.HOME = await import("node:os").then((os) => os.tmpdir());
  ...
  it("classifies TOKENS_LIMIT by unit: 3 → 5h, 6 → 7d", async () => {
    process.env.ZAI_API_KEY = FAKE_KEY;
