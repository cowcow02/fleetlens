...
| Tier | Signal | Precision | Coverage (orbit) | Use as |
|---|---|---|---|---|
...
When two URLs genuinely tie under this rule (equal position, equal frequency), the session is flagged as **multi-ticket** and both ticket IDs are stored as separate `ticket_sessions` rows. This is rare but legitimate — the orbit experiment contained 12 sessions that legitimately touched two real tickets (e.g., PR #116 → ORB-119 + ORB-37). Forcing a single winner would lose fidelity.

**Empirical precision of the disambiguation rule** (orbit dataset):
...
**PR #58 maps to ORB-66**, observed passively. In the orbit experiment, 163 of 231 URL-bearing sessions contained both sides, giving us that many mappings for free. A later session that contains only one side of the pair can be enriched from this passive index without any network call.
...
|---|---|---|
| **orbit/agentic-knowledge-system** | **184** | 3,452 |
...
```json
{
  ...
  "ticketSignals": {
    "urls": [
      {
        "url": "https://github.com/orbit/agentic-knowledge-system/pull/194",
        "type": "github-pr",
        "priority": 1,
        "extractedFrom": "tool_result:gh pr create"
      },
      {
        "url": "https://linear.app/orbit/issue/ORB-148",
        "type": "linear-issue",
        "priority": 2,
    ...
    "agentNameTicketHint": "orb-148",
    "slashCommandTicketHint": "ORB-148",
    "contentFrequencyTop": { "id": "ORB-148", "count": 47 },
    "gitBranch": "feat/ORB-148-cli-members-command"
  }
}
```
...
- **Multi-ticket blocks** (rare, ~3% in orbit) → two rows with `weight = 0.5` each (or proportional to content-frequency hit counts). Per-ticket rollups use `SUM(block.agent_time_ms * stbt.weight)` so team totals stay accurate without double-counting.
...
**Default enricher in v1: `gh-pr-list`**. Runs on the team server via `child_process`. Covers PR titles + merge state — the 91% of orbit tickets that had PRs.
...
```json
{
  ...
  "failedPermanent": 3,
  "abandoned": 1,
  "lastFailureReason": "not_authorized: gh not authed for orbit/private-repo"
}
```
...
After (Doc 4):  [ORB-148 api-svc 4.1h ────]*   ← "*" is the shipped marker
```
...
├──────────────────────────────────────────────────────────────┤
│ ORB-148  CLI members command              Alice   shipped *   │
│   Apr 12-15 · 4.1h agent time · 2 skills · 5 sessions         │
├──────────────────────────────────────────────────────────────┤
│ ORB-151  dashboard cards                  Bob     in progress │
│   Apr 14- · 2.3h agent time · 3 skills · 3 sessions           │
├──────────────────────────────────────────────────────────────┤
│ ORB-147  invitation email                 Alice   merged *    │
...
2. **Daemon aggregator extension**: implement URL extraction (same-repo filter, disambiguation rule), cwd worktree detection, agentName/teamName read, slash command detection, content frequency scan, git branch read. Ship the results in `ticketSignals`. Unit tests against orbit JSONL fixtures.
...
16. **End-to-end smoke test**: deploy fresh Docs 1/2/3/4, run 3 test daemons producing realistic orbit-like session data for 24 hours, verify auto-detect runs, tickets populate, insights emit, Gantt decorates correctly.
...
- **Ticket IDs** (e.g. `ORB-148`) — considered metadata, not content
...
## Appendix B — Validation: orbit experiment summary

The signal hierarchy and auto-detection were validated against **413 real JSONL sessions** from the orbit `agentic-knowledge-system` repo. Summary (full details in the monolith spec, `2026-04-15-team-edition-design.md`):
