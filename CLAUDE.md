...
- `packages/cli` — `fleetlens` on npm: CLI that manages the dashboard server, provides terminal stats, handles auto-updates.
...
```bash
pnpm -F fleetlens build    # Build CLI with esbuild
